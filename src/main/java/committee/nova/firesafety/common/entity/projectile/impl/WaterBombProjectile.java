...
public class WaterBombProjectile extends FunctionalProjectile {
    ...
    public void onRemovedFromWorld() {
        ...
        for (int i = 0; i < 10; i++) {
            level().addAlwaysVisibleParticle(CAMPFIRE_COSY_SMOKE, getX() + ThreadLocalRandom.current().nextDouble(-.3, .6), getY(), getZ() + ThreadLocalRandom.current().nextDouble(-.3, .6), 0, 0, 0);
        }
    }
}
